...
/**
 * TODO: should generated by 'kernel.tf'
 */
#define TNUM_LKMID (3) /* generated */
#define TNUM_LUMID (1) /* generated */
#define TNUM_LDMID (TNUM_LUMID + TNUM_LKMID)
const ID _dmloader_lum_domid_table[TNUM_LUMID] = {DOM1};
LDMCB _dmloader_ldmcb_table[TNUM_LDMID];

/**
 * In this file
 */

#define TMIN_LUMID (0) /* const */
#define TMIN_LKMID (TMIN_LUMID + TNUM_LUMID) /* const */
#define TMIN_LDMID (TMIN_LUMID)
#define IS_LUMID(x) ((x) >= TMIN_LUMID && (x) < (TMIN_LUMID + TNUM_LUMID))
#define IS_LKMID(x) ((x) >= TMIN_LKMID && (x) < (TMIN_LKMID + TNUM_LKMID))
#define LDMID(p_ldmcb)    ((ID)((p_ldmcb) - ldmcb_table))
#define VALID_LDMID(ldmid)  (TMIN_LDMID <= (ldmid) && (ldmid) < (TMIN_LDMID + TNUM_LDMID))
#define GET_LDMCB(ldmid) ((LDMCB*)(ldmcb_table + (ldmid)))
 ...
 */
static inline void elf32_load_segment(uint8_t *dest, const uint8_t *elf32_data, const Elf32_Phdr *seg) {
    if(seg == NULL) return;
...
static ER elf32_trans_load(elf32_load_trans *elt) {
    ...
    /**
     * Perform relocations and locate symbol table '.symtab'
     */
    ...
    /**
     * Locate entry point and exit point.
     */
    if(symtab == NULL || strtab == NULL) {
        syslog(LOG_ERROR, "[dmloader] '.symtab' section doesn't exist or is corrupted.");
        return E_OBJ;
    }
    elt->entry_point = NULL;
    elt->exit_point = NULL;
    for(size_t i = 0; i < symtab->sh_size / symtab->sh_entsize; ++i) {
        const Elf32_Sym* sym = ELF32_SYM(ehdr, symtab, i);

        if(ELF32_ST_TYPE(sym->st_info) == STT_FUNC) {
            if(strcmp(ELF32_STR_NAME(ehdr, strtab, sym->st_name), "module_initialize") == 0)
                elt->entry_point = elf32_vaddr_to_paddr(elt, sym->st_value);
            else if(strcmp(ELF32_STR_NAME(ehdr, strtab, sym->st_name), "module_terminate") == 0)
                elt->exit_point = elf32_vaddr_to_paddr(elt, sym->st_value);
        }
    }
    if(elt->entry_point == NULL || elt->exit_point == NULL) {
        syslog(LOG_ERROR, "[dmloader] Entry point or exit point doesn't exist.");
        return E_OBJ;
    }
